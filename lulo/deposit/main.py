"""
Lulo Deposit MCP Server
A FastMCP server for depositing funds to Lulo using Dialect Blink
"""
...
def lulo_deposit(symbol: str, amount: float, tx_sender_pubkey: str) -> DepositResponse:
    """
    Deposit funds to Lulo via Dialect Blink.
    ...
    try:
        blink_url = f"https://blink.lulo.fi/actions?amount={amount}&symbol={symbol}"
...
mcp.description = """
Lulo Deposit MCP enables depositing funds to Lulo via Dialect Blink API.
Provides a simple interface for executing deposits of Solana-based tokens.
"""

mcp.prompt_suggestions = [
    "Deposit 100 USDC to Lulo",
    "Deposit 1 SOL to my Lulo account",
]
