...
A FastMCP server for withdrawing from Drift vaults using Dialect Blink
"""
...
mcp = FastMCP("Drift Vaults Withdraw MCP")


class VaultsWithdrawInput(BaseModel):
    ...
    amount: float = Field(
        description="Amount to be withdrawn from the vault",
...
class VaultsWithdrawResponse(BaseModel):
    success: bool
    result: Optional[dict] = None
    error: Optional[str] = None


@mcp.tool(input_model=VaultsWithdrawInput)
def drift_vaults_withdraw(
    vault_type: str, vault_id: str, amount: float, tx_sender_pubkey: str
) -> VaultsWithdrawResponse:
    """
    Withdraw from a Drift vault via Dialect Blink.
    ...
    Returns:
        VaultsWithdrawResponse: Result of the withdraw request
    """
    if not BLINK_CLIENT_KEY:
        return VaultsWithdrawResponse(
            success=False, error="Missing BLINK_CLIENT_KEY environment variable"
        )

    try:
        blink_url = (
            f"https://drift.dial.to/vaults/{vault_type}/{vault_id}?action=withdraw&amount={amount}"
        )
        ...
        return VaultsWithdrawResponse(success=True, result=response.json())
        ...
        return VaultsWithdrawResponse(success=False, error=error_message)
    except Exception as e:
        return VaultsWithdrawResponse(success=False, error=f"Unexpected error: {str(e)}")


mcp.description = """
Drift Vaults Withdraw MCP enables withdrawing from Drift vaults via Dialect Blink API.
Provides a simple interface for withdrawing from strategy vaults or insurance fund vaults.
"""

mcp.prompt_suggestions = [
    "Withdraw 25 tokens from Drift strategy vault",
    "Withdraw 100 tokens from Drift insurance fund vault",
]
