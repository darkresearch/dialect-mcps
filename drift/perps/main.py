...
A FastMCP server for interacting with Drift Perpetuals using Dialect Blink
"""
...
def drift_perps(
    ...
    """
    Open a perpetual position on Drift via Dialect Blink.
    ...
    try:
        blink_url = f"https://drift.dial.to/perps/{market}/{side}?size={size}&leverage={leverage}"
...
mcp.description = """
Drift Perps MCP enables opening perpetual positions on Drift via Dialect Blink API.
Provides a simple interface for executing perpetual trades on Drift.
"""

mcp.prompt_suggestions = [
    "Open a 5x long position on SOL-PERP with 100 USD on Drift",
    "Create a short position on BTC-PERP with 10x leverage on Drift",
]
