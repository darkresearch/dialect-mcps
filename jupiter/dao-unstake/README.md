...
A FastMCP server for unstaking JUP tokens from Jupiter DAO via Dialect Blink.

## Features

- Simple MCP interface for unstaking JUP tokens from Jupiter DAO
- Configurable unstake amount
...
1. Clone the repository:
   ...
   cd dialect-mcps/jupiter/dao-unstake
   ```
...
```bash
fastmcp install main.py --name "Jupiter DAO Unstake"
```

### Example Requests

The MCP exposes a single `jupiter_dao_unstake` tool which accepts the following parameters:
