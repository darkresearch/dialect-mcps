...
A FastMCP server for staking JUP tokens in Jupiter DAO via Dialect Blink.

## Features

- Simple MCP interface for staking JUP tokens in Jupiter DAO
...
1. Clone the repository:
   ...
   cd dialect-mcps/jupiter/dao-stake
   ```
...
```bash
fastmcp install main.py --name "Jupiter DAO Stake"
```

### Example Requests

The MCP exposes a single `jupiter_dao_stake` tool which accepts the following parameters:
